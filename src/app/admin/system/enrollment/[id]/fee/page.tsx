...
export default function SystemEnrollmentFeePage() {
  ...
  const { data: enrollment, isLoading: enrollmentLoading, error: enrollmentError } = api.enrollment.getEnrollment.useQuery(
    { id },
    { enabled: !!id }
  );
  ...
  // Error state
  const hasErrors = enrollmentError || feeError || feeStructuresError || discountTypesError || challanTemplatesError;
    ...
    return (
      <div className="container mx-auto py-10">
        <Alert>
          ...
          <AlertTitle>Error Loading Fee Page</AlertTitle>
          <AlertDescription>
            <div className="space-y-2">
              {enrollmentError && <div>• Enrollment: {enrollmentError.message}</div>}
              {feeError && <div>• Fee: {feeError.message}</div>}
              {feeStructuresError && <div>• Fee Structures: {feeStructuresError.message}</div>}
              {discountTypesError && <div>• Discount Types: {discountTypesError.message}</div>}
              {challanTemplatesError && <div>• Challan Templates: {challanTemplatesError.message}</div>}
            </div>
          </AlertDescription>
        </Alert>
        <div className="mt-4">
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Enrollment Fee Management</h1>
