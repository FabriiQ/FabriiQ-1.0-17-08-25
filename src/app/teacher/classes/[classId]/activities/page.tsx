...
export default function ClassActivitiesPage() {
  ...
  const { data: activitiesData, isLoading: isLoadingActivities, error: activitiesError, refetch } = api.class.listActivities.useQuery(
    {
      classId,
      // Don't filter by status to get all activities
      take: 50, // Increase limit to show more activities
    },
    {
      ...
      onSuccess: (data) => {
        console.log('Activities query successful:', data);
      }
    }
  );
  ...
  const transformedActivities = useMemo(() => {
    ...
    console.log('Activities items:', activitiesData?.items);
    console.log('Activities total:', activitiesData?.total);
  ...
  }, [activitiesData, session?.user?.id, session?.user?.name]);
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="space-y-2 mb-4 text-sm text-muted-foreground">
              ...
              <p>• Activities data: {activitiesData ? 'Present' : 'None'}</p>
              <p>• Activities total: {activitiesData?.total || 0}</p>
              <p>• Activities items: {activitiesData?.items?.length || 0}</p>
