...
const ROUTE_CACHE_TTL = 5 * 60 * 1000; // 5 minutes for route decisions
const USER_TYPE_CACHE_TTL = 10 * 60 * 1000; // 10 minutes for user type caching
...
function institutionMiddleware(req: NextRequest, routeType: string) {
  ...
  if (routeType === 'skip' || routeType === 'public') {
    return null;
  }
 ...
 */
function validateUserAccess(userType: string, routeType: string, pathname: string): NextResponse | null {
  ...
  switch (userType) {
    case 'CAMPUS_TEACHER':
    case 'TEACHER':
      if (routeType !== 'teacher' && routeType !== 'protected') {
    ...
    case 'CAMPUS_STUDENT':
    case 'STUDENT':
      if (routeType !== 'student' && routeType !== 'protected') {
    ...
    default:
      // Unknown user types can't access teacher/student routes
      if (routeType === 'teacher' || routeType === 'student') {
  ...
  return redirectUrl ? NextResponse.redirect(new URL(redirectUrl, pathname)) : null;
}
...
export default withAuth(
  function middleware(req: NextRequest & { nextauth: { token: any } }) {
    ...
    const accessValidation = validateUserAccess(userType, routeType, pathname);
    if (accessValidation) {
